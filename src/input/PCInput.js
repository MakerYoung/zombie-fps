export class PCInput {
  constructor(el,state){ this.el=el; this.s=state; this.keys=new Set();
    ...
    this.timer=setInterval(()=>{state.move.x=(this.keys.has('KeyD')?1:0)-(this.keys.has('KeyA')?1:0);state.move.y=(this.keys.has('KeyW')?1:0)-(this.keys.has('KeyS')?1:0);state.sprint=this.keys.has('ShiftLeft')||this.keys.has('ShiftRight');state.crouch=this.keys.has('ControlLeft')||this.keys.has('ControlRight')||this.keys.has('KeyC');},16);
  }
}
